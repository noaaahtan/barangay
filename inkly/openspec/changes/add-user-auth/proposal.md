...
The application currently has no authentication — all endpoints are public. Adding login protects inventory data and enables per-user accountability. Four seed users (Noah, Fatima, Geelane, Marco) are needed so the team can log in immediately after deployment.
